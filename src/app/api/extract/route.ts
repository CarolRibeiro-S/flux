...
// A Anthropic só aceita esses 4 media types para imagem
const MEDIA_TYPES_VALIDOS = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'] as const
type MediaTypeValido = (typeof MEDIA_TYPES_VALIDOS)[number]
...
const EXTENSAO_PARA_MIME: Record<string, MediaTypeValido> = {
  ...
  webp: 'image/webp',
  gif: 'image/gif',
  // HEIC/HEIF não é suportado pela Anthropic — cai no fallback jpeg abaixo
}
...
// só confiamos no blobType se ele já for um dos 4 tipos que a Anthropic aceita.
function inferirMimeType(imagePath: string, blobType: string): MediaTypeValido {
